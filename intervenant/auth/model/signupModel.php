...
/*	if(strlen($_POST['pseudo']) < 3){
		...
		$stmt = $db->prepare('SELECT pseudo FROM intervenant_all WHERE pseudo = :pseudo');
...
//	}
	...
	if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) /*|| ! endsWith($_POST['email'],'@diese.org')*/){
		...
		$stmt = $db->prepare('SELECT mail FROM intervenant_all WHERE mail = :email');
	...
	if(isset($_POST['algerian'])){
		header('Location: http://www.viedemerde.fr/');
	}else{
